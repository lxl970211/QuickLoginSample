...
public class HttpUtil {
    ...
    public void requestHttp(final String url, final String params, final Response response, final String contentType) {
        ThreadPoolUtil.execute(new Runnable() {
            @Override
            public void run() {
                HttpUtil.this.requestHttp2(url, params, response, contentType);
            }
        });
    }
     ...
     * http request
     */
    public void requestHttp2(final String url, final String params, final Response response, final String contentType) {
...
//                            "application/json");
     ...
     * 解析网络请求结果
     */
    private void parseHttpResult(final String result, final int responseCode, final Response response) {
        try {
            timer.cancel();
        } catch (Exception ex) {
            ex.printStackTrace();
            ...
            if (responseCode != HttpURLConnection.HTTP_OK) {
                ...
                if (responseCode == 0) {
                    ...
                    ThreadUtil.runOnMainThread(new Runnable() {
                        @Override
                        public void run() {
                            response.onError(102102 + "", TextUtils.isEmpty(result) ? "网络异常" : result);
                        }
                    });
                }
            } else {
